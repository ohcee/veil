...
#include "modules/bulletproof/commitment_shim.h"
...
int secp256k1_bulletproof_rangeproof_prove(
    ...
    for (i = 0; i < n_commits; i++) {
        ...
        if (commits == NULL) {
            /* Calculate commitment from blinding factor. Veil's pedersen_ecmult
             * hardcodes the standard generator G for the blind (via ecmult_gen)
             * and takes a single value generator -- i.e. blind*G + value*value_gen,
             * matching Veil's native 5-arg pedersen_commit. gens->blinding_gen
             * must therefore be G for the proof to verify against this commitment. */
