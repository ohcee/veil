...
/* --- Multi-exponentiation engine (Strauss + Pippenger), backported additively
 * from the Grin-lineage secp256k1-zkp. Veil's standalone secp256k1_ecmult
 * (above) is kept; Grin's strauss-based single-ecmult is intentionally
 * excluded. Uses Veil's native ecmult_context + the backported scratch. --- */
...
#define WNAF_SIZE_MX(w) WNAF_SIZE_BITS(WNAF_BITS, w)
 ...
 *  The number becomes represented by sum(2^{wi} * wnaf[i], i=0..WNAF_SIZE_MX(w)+1) - return_val.
 *  It has the following guarantees:
 *  - each wnaf[i] is either 0 or an odd integer between -(1 << w) and (1 << w)
 *  - the number of words set is always WNAF_SIZE_MX(w)
...
static int secp256k1_wnaf_fixed(int *wnaf, const secp256k1_scalar *s, int w) {
    ...
    if (secp256k1_scalar_is_zero(s)) {
        for (pos = 0; pos < WNAF_SIZE_MX(w); pos++) {
            wnaf[pos] = 0;
        }
        return 0;
    }
    ...
    last_w = WNAF_BITS - (WNAF_SIZE_MX(w) - 1) * w;

    /* Store the position of the first nonzero word in max_pos to allow
     * skipping leading zeros when calculating the wnaf. */
    for (pos = WNAF_SIZE_MX(w) - 1; pos > 0; pos--) {
        int val = secp256k1_scalar_get_bits_var(work, pos * w, pos == WNAF_SIZE_MX(w)-1 ? last_w : w);
        if(val != 0) {
            break;
        }
        wnaf[pos] = 0;
...
static int secp256k1_ecmult_pippenger_wnaf(secp256k1_gej *buckets, int bucket_window, struct secp256k1_pippenger_state *state, secp256k1_gej *r, const secp256k1_scalar *sc, const secp256k1_ge *pt, size_t num) {
    size_t n_wnaf = WNAF_SIZE_MX(bucket_window+1);
    size_t np;
    size_t no = 0;
    int i;
    int j;
...
#endif
    size_t entry_size = sizeof(secp256k1_ge) + sizeof(secp256k1_scalar) + sizeof(struct secp256k1_pippenger_point_state) + (WNAF_SIZE_MX(bucket_window+1)+1)*sizeof(int);
...
static int secp256k1_ecmult_pippenger_batch(const secp256k1_ecmult_context *ctx, secp256k1_scratch *scratch, secp256k1_gej *r, const secp256k1_scalar *inp_g_sc, secp256k1_ecmult_multi_callback cb, void *cbdata, size_t n_points, size_t cb_offset) {
    ...
    state_space->wnaf_na = (int *) secp256k1_scratch_alloc(scratch, entries*(WNAF_SIZE_MX(bucket_window+1)) * sizeof(int));
    ...
    for(i = 0; (size_t)i < idx; i++) {
        ...
        for(j = 0; j < WNAF_SIZE_MX(bucket_window+1); j++) {
            state_space->wnaf_na[i * WNAF_SIZE_MX(bucket_window+1) + j] = 0;
        }
    }
...
#endif /* ENABLE_MODULE_BULLETPROOF */
